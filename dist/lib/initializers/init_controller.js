/* jshint node: true */
'use strict';
var Q = require('q'), _ = require('busyman'), Ziee = require('../../ziee'), debug = require('debug')('zigbee-shepherd:init');
var Coordinator = require('../model/coord'), Coordpoint = require('../model/coordpoint'), nvParams = require('../config/nv_start_options.js');
var init = {};
/*************************************************************************************************/
/*** Public APIs                                                                               ***/
/*************************************************************************************************/
...
init._startupCoord = function (controller) {
    var deferred = Q.defer(), stateChangeHdlr;
...
init._registerDelegators = function (controller, netInfo) {
    var coord = controller._coord, dlgInfos = [
    ...
    return controller.simpleDescReq(0, netInfo.ieeeAddr).then(function (devInfo) {
        ...
        _.forEach(dlgInfos, function (dlgInfo) {
            var dlgDesc = { profId: dlgInfo.profId, epId: dlgInfo.epId, devId: 0x0005, inClusterList: [], outClusterList: [] }, dlgEp = new Coordpoint(coord, dlgDesc, true), simpleDesc;
