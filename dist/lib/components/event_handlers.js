/* jshint node: true */
'use strict';
var Q = require('q'), EventEmitter = require('events'), _ = require('busyman'), Ziee = require('../../ziee'), ZSC = require('../../zstack-constants'), debug = {
...
var Device = require('../model/device'), Endpoint = require('../model/endpoint');
var handlers = {};
handlers.attachEventHandlers = function (shepherd) {
    var controller = shepherd.controller, hdls = {};
...
handlers.devIncoming = function (devInfo, resolve) {
    ...
    var self = this, dev = this._findDevByAddr(devInfo.ieeeAddr), clustersReqs = [];
    ...
    var processDev = Q.fcall(function () {
        ...
        if (!dev || !dev.hasOwnProperty('endpoints'))
            return dev;
        ...
        try {
            ...
            for (var i in dev.endpoints) {
                var ep = dev.getEndpoint(i), clusterList = ep.getClusterList();
            ...
            if (!basicEpInst || basicEpInst instanceof Error)
                return dev;
        ...
        interviewEvents.on('ind:interview', function (status) {
            if (status && status.endpoint)
                status.endpoint.total = numberOfEndpoints;
...
handlers.leaveInd = function (msg) {
    ...
    if (dev) {
        var ieeeAddr = dev.getIeeeAddr(), epList = _.cloneDeep(dev.epList);
