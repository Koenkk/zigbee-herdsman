...
const fs = require('fs');
var util = require('util'), EventEmitter = require('events');
var Q = require('q'), _ = require('busyman'), znp = require('../../cc-znp'), proving = require('proving'), ZSC = require('../../zstack-constants'), debug = {
...
var Zdo = require('./zdo'), query = require('./query'), bridge = require('./event_bridge.js'), init = require('../initializers/init_controller'), nvParams = require('../config/nv_start_options.js'), nvBackup = require('../config/nv_backup.js');
var Device = require('../model/device'), Coordpoint = require('../model/coordpoint');
...
Controller.prototype.start = function (callback) {
    var self = this, deferred = Q.defer(), readyLsn;
...
Controller.prototype.close = function (callback) {
    var self = this, deferred = Q.defer(), closeLsn;
    closeLsn = function () {
        deferred.resolve();
    };
...
Controller.prototype.reset = function (mode, callback) {
    var self = this, deferred = Q.defer(), startupOption = nvParams.startupOption.value[0];
    ...
    Q.fcall(function () {
        if (mode === 'soft' || mode === 1) {
            ...
            if (isZstack3) {
                steps = steps.concat([
                    function () {
                        return self.request('SYS', 'osalNvItemInit', nvParams.znpCfgItem3).delay(10).fail(function (err) {
                ...
                steps = steps.concat([
                    function () {
                        return self.request('SYS', 'osalNvItemInit', nvParams.znpCfgItem).delay(10).fail(function (err) {
                            return (err.message === 'rsp error: 9') ? null : Q.reject(err); // Success, item created and initialized
                        });
                    },
...
Controller.prototype.permitJoin = function (time, type, callback) {
    ...
    var self = this, addrmode, dstaddr;
...
Controller.prototype.remove = function (dev, cfg, callback) {
    ...
    var self = this, reqArgObj, rmChildren_reJoin = 0x00;
...
Controller.prototype.deregisterEp = function (loEp, callback) {
    var self = this, coordEps = this._coord.endpoints;
    if (!(loEp instanceof Coordpoint))
        throw new TypeError('loEp should be an instance of Coordpoint class.');
    return Q.fcall(function () {
...
Controller.prototype.checkNvParams = function (callback) {
    ...
    if (self._isZstack3x0 || self._isZstack30x) {
        ...
        steps.push(function () { return self.request('SYS', 'osalNvRead', nvParams.znpHasConfigured3).delay(10).fail(cb).then(cb); });
    }
    else {
        steps = steps.concat([
            function () {
                return self.request('SYS', 'osalNvRead', nvParams.znpHasConfigured).delay(10).then(function (rsp) {
                    if (!_.isEqual(bufToArray(rsp.value), nvParams.znpHasConfigured.value))
                        return Q.reject('reset');
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvParams.panId).delay(10).then(function (rsp) {
                    if (!_.isEqual(bufToArray(rsp.value), nvParams.panId.value))
                        return Q.reject('reset');
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvParams.extPanId).delay(10).then(function (rsp) {
                    if (!_.isEqual(bufToArray(rsp.value), nvParams.extPanId.value))
                        return Q.reject('reset');
                });
            },
        ]);
    ...
    steps = steps.concat([
        function () {
            return self.request('SYS', 'osalNvRead', nvParams.channelList).delay(10).then(function (rsp) {
                if (!_.isEqual(bufToArray(rsp.value), nvParams.channelList.value))
                    return Q.reject('reset');
            });
        },
        function () {
            return self.request('SYS', 'osalNvRead', nvParams.precfgkeysEnable).delay(10).then(function (rsp) {
    ...
    if (self._isZstack3x0) {
        steps.push(function () {
            return self.request('SYS', 'osalNvRead', nvParams.precfgkey3).delay(10).then(function (rsp) {
                if (!_.isEqual(bufToArray(rsp.value), nvParams.precfgkey3.value))
                    return Q.reject('reset');
            });
        });
    }
...
Controller.prototype.checkOnline = function (dev, callback) {
    var self = this, nwkAddr = dev.getNwkAddr(), ieeeAddr = dev.getIeeeAddr(), deferred = Q.defer();
    Q.fcall(function () {
...
Controller.prototype.endDeviceAnnceHdlr = function (data) {
    var self = this, joinTimeout, joinEvent = 'ind:incoming' + ':' + data.ieeeaddr, dev = this._shepherd._findDevByAddr(data.ieeeaddr);
...
Controller.prototype.backupCoordinator = function (path, callback) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        const steps = [
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_EXTADDR).delay(10).then((result) => {
                    backup.items.ZCD_NV_EXTADDR = Object.assign({}, nvBackup.ZCD_NV_EXTADDR, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_NIB).delay(10).then((result) => {
                    backup.items.ZCD_NV_NIB = Object.assign({}, nvBackup.ZCD_NV_NIB, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_EXTENDED_PAN_ID).delay(10).then((result) => {
                    backup.items.ZCD_NV_EXTENDED_PAN_ID = Object.assign({}, nvBackup.ZCD_NV_EXTENDED_PAN_ID, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_NWK_ACTIVE_KEY_INFO).delay(10).then((result) => {
                    backup.items.ZCD_NV_NWK_ACTIVE_KEY_INFO = Object.assign({}, nvBackup.ZCD_NV_NWK_ACTIVE_KEY_INFO, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_NWK_ALTERN_KEY_INFO).delay(10).then((result) => {
                    backup.items.ZCD_NV_NWK_ALTERN_KEY_INFO = Object.assign({}, nvBackup.ZCD_NV_NWK_ALTERN_KEY_INFO, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_APS_USE_EXT_PANID).delay(10).then((result) => {
                    backup.items.ZCD_NV_APS_USE_EXT_PANID = Object.assign({}, nvBackup.ZCD_NV_APS_USE_EXT_PANID, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_PRECFGKEY).delay(10).then((result) => {
                    backup.items.ZCD_NV_PRECFGKEY = Object.assign({}, nvBackup.ZCD_NV_PRECFGKEY, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_TCLK_TABLE_START).delay(10).then((result) => {
                    backup.items.ZCD_NV_TCLK_TABLE_START = Object.assign({}, nvBackup.ZCD_NV_TCLK_TABLE_START, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_CHANLIST).delay(10).then((result) => {
                    backup.items.ZCD_NV_CHANLIST = Object.assign({}, nvBackup.ZCD_NV_CHANLIST, { value: result.value.toJSON().data, len: result.len });
                });
            },
            function () {
                return self.request('SYS', 'osalNvRead', nvBackup.ZCD_NV_NWK_SEC_MATERIAL_TABLE_START).delay(10).then((result) => {
                    backup.items.ZCD_NV_NWK_SEC_MATERIAL_TABLE_START = Object.assign({}, nvBackup.ZCD_NV_NWK_SEC_MATERIAL_TABLE_START, { value: result.value.toJSON().data, len: result.len });
                });
            },
        ];
        Q.fcall(function () {
...
Controller.prototype.restoreCoordinator = function (path) {
    ...
    const ZCD_NV_NIB = Object.assign({}, backup.items.ZCD_NV_NIB, { initvalue: backup.items.ZCD_NV_NIB.value, initlen: backup.items.ZCD_NV_NIB.len });
    ...
    const steps = [
        ...
        function () {
            return self.request('SYS', 'osalNvItemInit', ZCD_NV_NIB).delay(10).fail(function (err) {
                return (err.message === 'rsp error: 9') ? null : Q.reject(err); // Success, item created and initialized
            });
        },
        ...
        function () { return self.request('SYS', 'osalNvWrite', nvParams.znpHasConfigured3).delay(10); },
        function () {
            return self.request('SYS', 'osalNvItemInit', nvParams.bdbNodeIsOnANetwork).delay(10).fail(function (err) {
                return (err.message === 'rsp error: 9') ? null : Q.reject(err); // Success, item created and initialized
            });
        },
        function () { return self.request('SYS', 'resetReq', { type: 0x01 }).delay(10); },
