/* jshint node: true */
'use strict';
var fs = require('fs'), util = require('util'), EventEmitter = require('events');
var Q = require('q'), _ = require('busyman'), zclId = require('../zcl-id'), proving = require('proving'), Objectbox = require('objectbox'), debug = { shepherd: require('debug')('zigbee-shepherd') };
var init = require('./initializers/init_shepherd'), zutils = require('./components/zutils'), Controller = require('./components/controller'), eventHandlers = require('./components/event_handlers');
var Device = require('./model/device'), Coordinator = require('./model/coord'), Group = require('./model/group'), Coordpoint = require('./model/coordpoint');
/*************************************************************************************************/
/*** ZShepherd Class                                                                           ***/
/*************************************************************************************************/
function ZShepherd(path, opts) {
    ...
    this.acceptDevIncoming = function (devInfo, callback) {
    ...
    this.on('ind:statusChange', function (ep, cId, payload, msg) {
        var cIdString = zclId.cluster(cId), notifData = {
            cid: '',
            zoneStatus: null
        };
...
ZShepherd.prototype.stop = function (callback) {
    var self = this, devbox = this._devbox;
...
ZShepherd.prototype.reset = function (mode, callback) {
    var self = this, devbox = this._devbox, removeDevs = [];
...
ZShepherd.prototype.mount = function (zApp, callback) {
    var self = this, deferred = (callback && Q.isPromise(callback.promise)) ? callback : Q.defer(), coord = this.controller._coord, mountId, loEp;
...
ZShepherd.prototype.list = function (ieeeAddrs) {
    ...
    foundDevs = _.map(ieeeAddrs, function (ieeeAddr) {
        ...
        var devInfo, found = self._findDevByAddr(ieeeAddr);
        if (found)
...
ZShepherd.prototype.lqi = function (ieeeAddr, callback) {
    proving.string(ieeeAddr, 'ieeeAddr should be a string.');
    var self = this, dev = this._findDevByAddr(ieeeAddr);
    return Q.fcall(function () {
        if (dev)
    ...
    const processResponse = function (parent) {
        return function (data) {
            ...
            data.forEach(function (devinfo) {
                ...
                if (childIeeeAddr == "0x0000000000000000")
                    return;
...
ZShepherd.prototype._registerDev = function (dev, callback) {
    var devbox = this._devbox, oldDev;
...
ZShepherd.prototype._attachZclMethods = function (ep) {
    ...
    if (ep.constructor.name === 'Zive') {
        ...
        ep.read = function (cId, attrId, callback) {
            var deferred = Q.defer(), attr = zclId.attr(cId, attrId);
        ...
        ep.write = function (cId, attrId, data, callback) {
            var deferred = Q.defer(), attr = zclId.attr(cId, attrId), attrType = zclId.attrType(cId, attrId).value;
        ...
        ep.report = function (cId, attrId, minInt, maxInt, repChange, callback) {
            var deferred = Q.defer(), coord = self.controller._coord, dlgEp = coord.getDelegator(ep.getProfId()), cfgRpt = true, cfgRptRec, attrIdVal, attrTypeVal;
...
ZShepherd.prototype._updateFinalizer = function (ep, cId, attrs, reported) {
    ...
    var self = this, cIdString = zclId.cluster(cId), clusters = ep.getClusters().dumpSync();
    cIdString = cIdString ? cIdString.key : cId;
    Q.fcall(function () {
        if (attrs) {
            var newAttrs = {};
        ...
        var oldAttrs = clusters[cIdString].attrs, diff = zutils.objectDiff(oldAttrs, newAttrs);
        if (!_.isEmpty(diff)) {
