'use strict';
var _ = require('busyman'), zclId = require('zcl-id'), utils = require('./utils'), Spec = require('./spec');
function Ziee() {
    this.zapp = null;
}
/*************************************************************************************************/
...
Ziee.prototype.has = function (cid, sid, rid) {
    var cidKey = utils.getCidKey(cid), cluster = this[cidKey], spec, resrc, has = !!cluster;
...
Ziee.prototype.findSpec = function (cid, sid) {
    var cidkey = utils.getCidKey(cid), target = this[cidkey];
    if (!utils.isValidArgType(sid))
        throw new TypeError('sid should be given with a number or a string.');
    if (target)
        target = target[sid];
...
Ziee.prototype.get = function (cid, sid, rid) {
    var spec = this.findSpec(cid, sid), target;
    if (spec)
        target = spec.get(rid);
    return target;
};
Ziee.prototype.set = function (cid, sid, rid, value) {
    var spec = this.findSpec(cid, sid), set = false;
    if (spec) {
        spec.set(rid, value);
        set = true;
    }
...
Ziee.prototype.dumpSync = function (cid, sid) {
    var dumped, target, dumpType = 'clusters';
