...
zclId.profile = function (profId) {
    ...
    var profNumber = parseInt(profId), profItem;
    if (!isNaN(profNumber))
        profId = profNumber;
...
zclId.device = function (profId, devId) {
    ...
    var profNumber = parseInt(profId), devNumber = parseInt(devId), profItem, devItem;
    if (!isNaN(profNumber))
        profId = profNumber;
    if (!isNaN(devNumber))
        devId = devNumber;
...
zclId.cluster = function (cId) {
    ...
    var cNumber = parseInt(cId), cItem;
    if (!isNaN(cNumber))
        cId = cNumber;
...
zclId.foundation = function (cmdId) {
    ...
    var cmdNumber = parseInt(cmdId), cmdItem;
    if (!isNaN(cmdNumber))
        cmdId = cmdNumber;
...
zclId.functional = function (cId, cmdId) {
    ...
    if (!isValidArgType(cmdId))
        throw new TypeError('cmdId should be a number or a string.');
    var cNumber = parseInt(cId), cmdNumber = parseInt(cmdId), cItem, cmdItem, cInfo;
    if (!isNaN(cNumber))
        cId = cNumber;
    if (!isNaN(cmdNumber))
        cmdId = cmdNumber;
...
zclId.getCmdRsp = function (cId, rspId) {
    ...
    var cNumber = parseInt(cId), cmdNumber = parseInt(rspId), cItem, cmdItem, cInfo;
    if (!isNaN(cNumber))
        cId = cNumber;
    if (!isNaN(cmdNumber))
        rspId = cmdNumber;
...
zclId.attrList = function (cId) {
    ...
    if (!isValidArgType(cId))
        throw new TypeError('cId should be a number or a string.');
    var cItem = zclId.cluster(cId), clst = cItem ? zclId._getCluster(cItem.key) : undefined;
    if (!cItem || !clst)
        return;
...
zclId.attr = function (cId, attrId) {
    ...
    var cNumber = parseInt(cId), attrNumber = parseInt(attrId), cItem, attrItem, cInfo;
    if (!isNaN(cNumber))
        cId = cNumber;
    if (!isNaN(attrNumber))
        attrId = attrNumber;
...
zclId.attrType = function (cId, attrId) {
    ...
    if (!isValidArgType(attrId))
        throw new TypeError('attrId should be a number or a string.');
    var cNumber = parseInt(cId), attrNumber = parseInt(attrId), cItem, attrItem, attrName, attrType, cInfo;
    if (!isNaN(cNumber))
        cId = cNumber;
    if (!isNaN(attrNumber))
        attrId = attrNumber;
...
zclId.dataType = function (type) {
    ...
    var typeNumber = parseInt(type), typeItem;
    if (!isNaN(typeNumber))
        type = typeNumber;
...
zclId.status = function (status) {
    ...
    var statusNumber = parseInt(status), statusItem;
