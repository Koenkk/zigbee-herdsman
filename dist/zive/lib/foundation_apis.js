/* jshint node: true */
'use strict';
var _ = require('busyman'), zclId = require('zcl-id');
var zApp, foundApis = {};
foundApis.read = function (clusters, cId, payload, afMsg, callback) {
    var readFuncs = [];
    _.forEach(payload, function (readRec) {
        var attrId = readRec.attrId, attrType = zclId.attrType(cId, attrId).key, rptCfg;
...
foundApis.write = function (clusters, cId, payload, afMsg, callback) {
    ...
    _.forEach(payload, function (writeRec) {
        var attrId = writeRec.attrId, attrType = zclId.attrType(cId, attrId).key, rptCfg;
        writeFuncs.push(function (cb) {
            var acl = clusters.get(cId, 'attrs', attrId), writeStatusRec = {
                attrId: attrId,
                status: null
            };
            if (acl === 'R') {
...
foundApis.configReport = function (clusters, cId, payload, afMsg, callback) {
    ...
    _.forEach(payload, function (attrRptCfgRec) {
        var attrId = attrRptCfgRec.attrId, attrType = zclId.attrType(cId, attrId).key, cfg = clusters.get(cId, 'rptCfgs', attrId), attrStatusRec = {
            attrId: attrId,
...
foundApis.readReportConfig = function (clusters, cId, payload, afMsg, callback) {
    ...
    _.forEach(payload, function (attrRec) {
        var attrId = attrRec.attrId, attrType = zclId.attrType(cId, attrId).value, direction = attrRec.direction, cfg = clusters.get(cId, 'rptCfgs', attrId), attrRptCfgRec = {
            attrId: attrId,
            direction: direction,
            status: null
        };
...
foundApis.discover = function (clusters, cId, payload, afMsg, callback) {
    var attrs = clusters.dumpSync(cId, 'attrs'), startId = payload.startAttrId, maxNums = payload.maxAttrIds, discRsp = {
        discComplete: 1,
        attrInfos: []
    };
    _.forEach(attrs, function (info, id) {
        var attrId = zclId.attr(cId, id).value, attrInfo = {
            attrId: attrId,
            dataType: null
        };
...
function execAsyncFuncs(funcs, callback) {
    var count = 0, flag = false, allResult = [];
    if (_.isEmpty(funcs))
...
function isAnalog(dataType) {
    var type = zclId.dataType(dataType).value, analogDigital;
...
//# sourceMappingURL=foundation_apis.js.map