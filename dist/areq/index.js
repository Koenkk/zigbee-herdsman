...
Areq.prototype.register = function (evt, deferred, listener, time) {
    var self = this, registered = false, areqTimeout = time || this._areqTimeout;
...
Areq.prototype.deregister = function (evt) {
    var rec = this.getRecord(evt), emitter = this._emitter;
    if (rec) {
        if (rec.deferred)
            rec.deferred = null;
        if (rec.listener) {
...
Areq.prototype.resolve = function (evt, value) {
    var rec = this.getRecord(evt), deferred = rec ? rec.deferred : null;
