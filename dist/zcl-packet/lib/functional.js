...
function FuncPayload(clusterId, direction, cmd) {
    var cluster = zclId.cluster(clusterId), command, params;
...
FuncPayload.prototype.parse = function (zclBuf, callback) {
    var chunkRules = [], err, params, parser, parseTimeout;
...
FuncPayload.prototype.frame = function (args) {
    var params;
    ...
    args.forEach(function (arg, idx) {
        var type = arg.type, val = arg.value, idxarr, k = 0;
        switch (type) {
            case 'int8':
            case 'uint8':
            case 'int16':
            ...
            case 'longaddr': // string '0x00124b00019c2ee9'
                var msb = parseInt(val.slice(2, 10), 16), lsb = parseInt(val.slice(10), 16);
            ...
            case 'dynUint24': // [ x, y, z, ... ]
                for (idxarr = 0; idxarr < val.length; idxarr += 1) {
                    var value = val[idxarr], msb24 = (value & 0xff0000) >> 16, mid24 = (value & 0xff00) >> 8, lsb24 = (value & 0xff);
            ...
            case 'locationbuffer': // [ '0x00124b00019c2ee9', int16, int16, int16, int8, uint8, ... ]
                for (idxarr = 0; idxarr < (val.length) / 6; idxarr += 1) {
                    var msbaddr = parseInt(val[k].slice(2, 10), 16), lsbaddr = parseInt(val[k].slice(10), 16);
...
/*************************************************************************************************/
var rules1 = ['preLenUint8', 'preLenUint16', 'preLenUint32'], rules2 = ['dynUint8', 'dynUint16', 'dynUint24', 'dynUint32', 'zonebuffer', 'extfieldsets', 'locationbuffer'];
    ...
    switch (type) {
        ...
        case 'dynUint24':
            for (i = 0; i < buf.length; i += 3) {
                var lsb = buf.readUInt16LE(i), msb = buf.readUInt8(i + 2), val = (msb << 16) + lsb;
                arr.push(val);
            }
            break;
        case 'dynUint32':
