/* jshint node: true */
'use strict';
var Concentrate = require('concentrate'), DChunks = require('../../dissolve-chunks'), ru = DChunks().Rule();
var FoundPayload = require('./foundation'), FuncPayload = require('./functional');
var zcl = {};
zcl.parse = function (zclBuf, clusterId, callback) {
    var zclObj, zclFrame = new ZclFrame();
    if (!Buffer.isBuffer(zclBuf))
        throw new TypeError('zclBuf should be a buffer.');
...
zcl.header = function (buf) {
    ...
    var i = 0, headByte = buf.readUInt8(0), header = {
        frameCntl: {
...
ZclFrame.prototype.frame = function (frameCntl, manufCode, seqNum, cmdId, payload) {
    ...
    if (!isNumber(seqNum))
        throw new TypeError('seqNum should be a number');
    var frameCntlOctet = (frameCntl.frameType & 0x03) | ((frameCntl.manufSpec << 2) & 0x04) | ((frameCntl.direction << 3) & 0x08) | ((frameCntl.disDefaultRsp << 4) & 0x10), dataBuf = Concentrate().uint8(frameCntlOctet);
