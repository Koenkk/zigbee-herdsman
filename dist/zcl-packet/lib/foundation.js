/* jshint node: true */
'use strict';
var Concentrate = require('concentrate'), DChunks = require('../../dissolve-chunks'), ru = DChunks().Rule(), zclId = require('../../zcl-id');
...
FoundPayload.prototype.parse = function (zBuf, callback) {
    var self = this, getObjCB, parsedData;
...
FoundPayload.prototype.frame = function (payload) {
    var self = this, dataBuf = Concentrate();
    switch (this.cmd) {
        case 'defaultRsp':
        case 'discover':
    ...
    switch (this.cmd) {
        ...
        default:
            this.params.forEach(function (param) {
                var paramName = param.name, paramType = param.type;
...
FoundPayload.prototype._getObj = function (buf, callback) {
    var chunkRules = [], parser, parseTimeout, knownBufLen = zclmeta.foundation.get(this.cmd).knownBufLen, result = {
        data: null,
        leftBuf: null
    };
    if (buf.length === 0) {
...
function getChunkBuf(rule, arg) {
    var dataBuf = Concentrate(), type, i = 0;
    switch (rule) {
        case 'multi':
...
function getDataType(dataType) {
    var type = ensureDataTypeString(dataType), newDataType;
    switch (type) {
        case 'data8':
        case 'boolean':
        case 'bitmap8':
...
function getDataTypeBuf(type, value) {
    var dataBuf = Concentrate(), string, strLen;
    switch (type) {
        case 'uint8':
        case 'int8':
        case 'uint16':
        ...
        case 'int56':
            ...
            break;
        case 'uint64':
            var msb = parseInt(value.slice(2, 10), 16), lsb = parseInt(value.slice(10), 16);
...
function isDataAnalogDigital(dataType) {
    var type = zclId.dataType(ensureDataTypeString(dataType)).value, analogDigital;
    ...
    else if ((type > 0x1f && type < 0x30) || //UNSIGNED_INT, SIGNED_INT
...
ru.clause('int24', function (name) {
    this.uint16('lsb').uint8('msb').tap(function () {
        var value, sign = (this.vars.msb & 0x80) >> 7;
        value = ((this.vars.msb & 0x7F) * 65536) + this.vars.lsb;
        if (sign)
            this.vars[name] = -(0x7FFFFF - value + 1);
        else
            this.vars[name] = value;
        delete this.vars.lsb;
        delete this.vars.msb;
    ...
    this.uint8('elmType').uint16('numElms').tap(function () {
        if (!this.vars.numElms) {
            ...
            this.loop('elmVals', function (end) {
                ...
                count += 1;
                if (count === this.vars.numElms)
                    end();
            }).tap(function () {
                var tempArr = [];
                this.vars.elmVals.forEach(function (elmVal) {
                    if (elmVal.data)
                        tempArr.push(elmVal.data);
                });
...
ru.clause('multi', function (name) {
    ...
    this.tap(name, function () {
        var type = zclId.dataType(this.vars.dataType).key, dataType;
        if (type === 'array' || type === 'set' || type === 'bag') {
            ru.attrVal()(this);
        }
        else if (type === 'struct') {
        ...
        if (flag)
            this.vars[name] = this.vars[name][name];
    });
});
...
function buf2Str(buf) {
    var bufLen = buf.length, val, strChunk = '0x';
