...
function ZpiObject(subsys, cmd, args) {
    var subsystem = zmeta.Subsys.get(subsys), command, reqParams;
...
ZpiObject.prototype.parse = function (type, bufLen, zBuf, callback) {
    var chunkRules = [], err, rspParams, parser, parseTimeout;
...
ZpiObject.prototype.frame = function () {
    ...
    this.args.forEach(function (arg, idx) {
        var type = arg.type, val = arg.value;
        switch (type) {
            case 'uint8':
            case 'uint16':
            case 'uint32':
            ...
            case 'listbuffer': // [ 0x0001, 0x0002, 0x0003, ... ]
                var tempBuf = new Buffer(val.length * 2), idxbuf;
...
ru.clause('nwklistbuffer', function (name, bufLen) {
    this.buffer(name, bufLen - 6).tap(function () {
        var buf = this.vars[name], list = [], listcount, getList, start = 0, end, len, i;
        if (name === 'networklist') {
...
ru.clause('preLenBeaconlist', function (name) {
    this.uint8(name).tap(function () {
        this.buffer('preLenData', 21 * (this.vars[name])).tap(function () {
            var buf = this.vars.preLenData, list = [], len = 21, start = 0, end = 21, i;
...
function bindTableList(buf, listcount) {
    var itemObj, list = [], len = 21, start = 0, end = len, i;
