...
CcZnp.prototype._sendSREQ = function (argObj, callback) {
    ...
    var self = this, payload = argObj.frame(), sreqTimeout, srspEvt = 'SRSP:' + argObj.subsys + ':' + argObj.cmd;
    if (!payload) {
...
CcZnp.prototype._sendAREQ = function (argObj, callback) {
    // subsys: String, cmd: String
    var self = this, payload = argObj.frame();
    if (!payload) {
        callback(new Error('Fail to build frame'));
        return;
    }
...
CcZnp.prototype._mtIncomingDataHdlr = function (err, data) {
    ...
    var rxEvt, msg, subsys = data.subsys, cmd = data.cmd, result = data.payload;
    if (data.type === 'SRSP') {
