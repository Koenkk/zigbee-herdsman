...
class Device extends Entity {
    ...
    public async onZclData(dataPayload: AdapterEvents.ZclDataPayload, endpoint: Endpoint): Promise<void> {
        ...
        if (!dataPayload.wasBroadcast && !disableDefaultResponse && !isDefaultResponse && !commandHasResponse &&
            ...
            try {
                //let defaultresponse_direction = 1 - frame.Header.frameControl.direction;
    ...
    private async interviewInternal(): Promise<void> {
        ...
        if (this.manufacturerID === 4619 && this._type === 'EndDevice') {
            ...
            try {
                const endpoint2 = Endpoint.create(1, undefined, undefined, [], [], this.networkAddress, this.ieeeAddr);
                const result = await endpoint2.read('genBasic', ['modelId', 'manufacturerName'], 
                    {sendWhen: 'immediate'});
        ...
        if (matchDescriptors.endpoints !==[])   {
            ...
            for (const endpointID1 of matchDescriptors.endpoints) {
                ...
                //this.save();
                const endpoint1 = Endpoint.create(endpointID1, undefined, undefined, [], [], this.networkAddress, this.ieeeAddr);
                let writedIEEEAddr = false;
                for (let attempt1 = 0; attempt1 < 6; attempt1++) {
                    await endpoint1.write('ssIasZone', {'iasCieAddr': coordinator.ieeeAddr}, {sendWhen: 'immediate'});
                    await Wait(500);
                    const writedIEEEAddrresult = await endpoint1.read('ssIasZone', ['iasCieAddr'], 
                        {sendWhen: 'immediate'});
                    await Wait(500);
                    debug.log(`Interview - IAS - after writing iasCieAddr (${attempt1}): '${JSON.stringify(writedIEEEAddr)}'`);
                ...
                let enrolled = false;
                for (let attempt2 = 0; attempt2 < 6; attempt2++) {
                    await Wait(500);
                    const stateAfter = await endpoint1.read('ssIasZone', ['zoneState', 'zoneType'], {sendWhen: 'immediate'});
                    //await endpoint1.read('ssIasZone', ['zoneStatus', 'currentZoneSensitivityLevel','tuya_iasZone_1','tuya_iasZone_2'], {sendWhen: 'immediate'});
                    debug.log(`Interview - IAS - after enrolling state (${attempt2}): '${JSON.stringify(stateAfter)}'`);
                ...
                if (enrolled) {
                    debug.log(`Interview - IAS successfully enrolled '${this.ieeeAddr}' endpoint '${endpoint1.ID}'`);
                } else {
                    throw new Error(
