...
const cLogger = logger.child({service: 'zigbee-herdsman:controller'});
...
async function catcho(func: () => Promise<void>, errorMessage: string): Promise<void> {
    try {
        ...
        cLogger.error(`${errorMessage}: ${error}`);
    }
}
...
class Controller extends events.EventEmitter {
    ...
    public async start(): Promise<AdapterTsType.StartResult> {
        ...
        cLogger.debug(`Starting with options '${JSON.stringify(this.options)}'`);
        const startResult = await this.adapter.start();
        cLogger.debug(`Started with result '${startResult}'`);
        ...
        // log injection
        cLogger.debug(`Injected database: ${this.database != null}, adapter: ${this.adapter != null}`);
        ...
        if (startResult === 'reset') {
            ...
            cLogger.debug('Clearing database...');
        ...
        if (Device.byType('Coordinator').length === 0) {
            cLogger.debug('No coordinator in database, querying...');
            Device.create(
        ...
        if (databaseCoordinator.ieeeAddr !== coordinator.ieeeAddr) {
            cLogger.info(`Coordinator address changed, updating to '${coordinator.ieeeAddr}'`);
    ...
    public async backup(): Promise<void> {
        ...
        if (this.options.backupPath && await this.adapter.supportsBackup()) {
            cLogger.debug('Creating coordinator backup');
    ...
    private onNetworkAddress(payload: AdapterEvents.NetworkAddressPayload): void {
        cLogger.debug(`Network address '${payload.ieeeAddr}'`);
        const device = Device.byIeeeAddr(payload.ieeeAddr);

        if (!device) {
            cLogger.debug(`Network address is from unknown device '${payload.ieeeAddr}'`);
            return;
        }
        ...
        if (device.networkAddress !== payload.networkAddress) {
            cLogger.debug(`Device '${payload.ieeeAddr}' got new networkAddress '${payload.networkAddress}'`);
    ...
    private onDeviceAnnounce(payload: AdapterEvents.DeviceAnnouncePayload): void {
        cLogger.debug(`Device announce '${payload.ieeeAddr}'`);
        const device = Device.byIeeeAddr(payload.ieeeAddr);

        if (!device) {
            cLogger.debug(`Device announce is from unknown device '${payload.ieeeAddr}'`);
            return;
        }
        ...
        if (device.networkAddress !== payload.networkAddress) {
            cLogger.debug(`Device '${payload.ieeeAddr}' announced with new networkAddress '${payload.networkAddress}'`);
            device.networkAddress = payload.networkAddress;
            device.save();
        }
    ...
    private onDeviceLeave(payload: AdapterEvents.DeviceLeavePayload): void {
        ...
        if (!device) {
            cLogger.debug(`Device leave is from unknown or already deleted device '${payload.ieeeAddr ?? payload.networkAddress}'`);
            return;
        }

        cLogger.debug(`Removing device from database '${device.ieeeAddr}'`);
        device.removeFromDatabase();
    ...
    private async onDeviceJoinedGreenPower(payload: GreenPowerDeviceJoinedPayload): Promise<void> {
        ...
        if (!device) {
            cLogger.debug(`New green power device '${ieeeAddr}' joined`);
            cLogger.debug(`Creating device '${ieeeAddr}'`);
    ...
    private async onDeviceJoined(payload: AdapterEvents.DeviceJoinedPayload): Promise<void> {
        ...
        if (this.options.acceptJoiningDeviceHandler) {
            if (!(await this.options.acceptJoiningDeviceHandler(payload.ieeeAddr))) {
                cLogger.debug(`Device '${payload.ieeeAddr}' rejected by handler, removing it`);
                await catcho(() => this.adapter.removeDevice(payload.networkAddress, payload.ieeeAddr), 
                    'Failed to remove rejected device');
                return;
            } else {
                cLogger.debug(`Device '${payload.ieeeAddr}' accepted by handler`);
            }
        }

        let device = Device.byIeeeAddr(payload.ieeeAddr, true);
        if (!device) {
            cLogger.debug(`New device '${payload.ieeeAddr}' joined`);
            cLogger.debug(`Creating device '${payload.ieeeAddr}'`);
            device = Device.create(
        ...
        if (!device.interviewCompleted && !device.interviewing) {
            ...
            try {
                ...
                cLogger.error(`Interview failed for '${device.ieeeAddr} with error '${error}'`);
    ...
    private async onZclOrRawData(
        ...
        cLogger.debug(`Received '${dataType}' data '${JSON.stringify(logDataPayload)}'`);
        ...
        if (device?.manufacturerName === 'LUMI' && device?.type == 'Router' && dataPayload.groupID) {
            cLogger.debug(`Handling re-transmitted Xiaomi message ${device.networkAddress} -> ${dataPayload.groupID}`);
            ...
            if (frame.isGlobal()) {
                ...
                if (frame.isSpecific()) {
                    if (Events.CommandsLookup[command.name]) {
                        ...
                        cLogger.debug(`Skipping command '${command.name}' because it is missing from the lookup`);
                    }
                }
            }
