...
const cLogger = logger.child({service: 'zigbee-herdsman:controller:touchlink'});
...
class Touchlink {
        ...
        try {
            for (const channel of scanChannels) {
                ...
                try {
                    ...
                    cLogger.debug(`Got scan response on channel '${channel}' of '${response.address}'`);
    ...
    public async identify(ieeeAddr: string, channel: number): Promise<void> {
        ...
        try {
            ...
            cLogger.debug(`Got scan response on channel '${channel}'`);
    ...
    public async factoryReset(ieeeAddr: string, channel: number): Promise<boolean> {
        ...
        try {
            ...
            cLogger.debug(`Reset to factory new '${ieeeAddr}'`);
