...
const cLogger = logger.child({service: 'zigbee-herdsman:deconz:driver'});
...
class Driver extends events.EventEmitter {
    ...
    public constructor(path: string) {
        ...
        setInterval(() => {
            that.writeParameterRequest(0x26, 600) // reset watchdog // 10 minutes
                ...
                .catch(error => {
                    //try again
                    cLogger.debug("try again to reset watchdog");
                    that.writeParameterRequest(0x26, 600)
                    .then(result => {})
                    .catch(error => {cLogger.debug("warning watchdog was not reset");});
                });
             }, (1000 * 60 * 8)); // 8 minutes
    ...
    protected catchPromise(val: any) {
        return Promise.resolve(val)
            .catch(err => cLogger.debug(`Promise was caught with reason: ${err}`));
    }

    public setDelay(delay: number): void {
        cLogger.debug(`Set delay to ${delay}`);
        this.DELAY = delay;
    ...
    public openSerialPort(baudrate: number): Promise<void> {
        cLogger.debug(`Opening with ${this.path}`);
    ...
    public readParameterRequest(parameterId: number) : Promise<Command> {
        ...
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push read parameter request to queue. seqNr: ${seqNumber} paramId: ${parameterId}`);
            const ts = 0;
    ...
    public writeParameterRequest(parameterId: number, parameter: parameterT) : Promise<void> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push write parameter request to queue. seqNr: ${seqNumber} paramId: ${parameterId} parameter: ${parameter}`);
            const ts = 0;
    ...
    public readFirmwareVersionRequest() : Promise<number[]> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push read firmware version request to queue. seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    private sendWriteParameterRequest(parameterId: number, value: parameterT, seqNumber: number) {
        ...
        //cLogger.debug("SEND WRITE_PARAMETER Request - parameter id: " + parameterId + " value: " + value.toString(16) + " length: " + parameterLength);
    ...
    private sendRequest(buffer: number[]) {
        ...
        if ( this.portType === 'serial') {
            this.serialPort.write(slipframe, function(err) {
                if (err) {
                    cLogger.debug("Error writing serial Port: " + err.message);
                }
            });
        } else {
            this.socketPort.write(slipframe, function(err) {
                if (err) {
                    cLogger.debug("Error writing socket Port: " + err.message);
                }
            });
        }
    }
    ...
    private processQueue() {
        ...
        switch (req.commandId) {
            case PARAM.PARAM.FrameType.ReadParameter:
                cLogger.debug(`send read parameter request from queue. seqNr: ${req.seqNumber} paramId: ${req.parameterId}`);
                this.sendReadParameterRequest(req.parameterId, req.seqNumber);
                break;
            case PARAM.PARAM.FrameType.WriteParameter:
                cLogger.debug(`send write parameter request from queue. seqNr: ${req.seqNumber} paramId: ${req.parameterId} param: ${req.parameter}`);
                this.sendWriteParameterRequest(req.parameterId, req.parameter, req.seqNumber);
                break;
            case PARAM.PARAM.FrameType.ReadFirmwareVersion:
                cLogger.debug(`send read firmware version request from queue. seqNr: ${req.seqNumber}`);
                this.sendReadFirmwareVersionRequest(req.seqNumber);
                break;
            case PARAM.PARAM.FrameType.ReadDeviceState:
                cLogger.debug(`send read device state from queue. seqNr: ${req.seqNumber}`);
                this.sendReadDeviceStateRequest(req.seqNumber);
                break;
            case PARAM.PARAM.NetworkState.CHANGE_NETWORK_STATE:
                cLogger.debug(`send change network state request from queue. seqNr: ${req.seqNumber}`);
    ...
    private async processBusyQueue() {
        ...
        while (i--) {
            ...
            if ((now - req.ts) > 10000) {
                cLogger.debug(`Timeout for request - CMD: 0x${req.commandId.toString(16)} seqNr: ${req.seqNumber}`);
                ...
                if (timeoutCounter >= 2) {
                    timeoutCounter = 0;
                    cLogger.debug("too many timeouts - restart serial connecion");
                    if (this.serialPort?.isOpen) {
                        this.serialPort.close();
                    }
                    if (this.socketPort) {
    ...
    public changeNetworkStateRequest(networkState: number) : Promise<void> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push change network state request to apsQueue. seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    private deviceStateRequest() {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`DEVICE_STATE Request - seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    private async checkDeviceStatus(currentDeviceStatus: number) {
        ...
        cLogger.debug("networkstate: " + networkState + " apsDataConfirm: " + this.apsDataConfirm + " apsDataIndication: " + this.apsDataIndication +
    ...
    private async handleDeviceStatus() {
        if (this.apsDataConfirm === 1) {
            try {
                cLogger.debug("query aps data confirm");
                this.apsDataConfirm = 0;
        ...
        if (this.apsDataIndication === 1) {
            try {
                cLogger.debug("query aps data indication");
                this.apsDataIndication = 0;
    ...
    private readReceivedDataRequest() : Promise<void> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push read received data request to apsQueue. seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    public enqueueSendDataRequest(request: ApsDataRequest) : Promise<void | ReceivedDataResponse> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push enqueue send data request to apsQueue. seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    private querySendDataStateRequest() : Promise<void> {
        const seqNumber = this.nextSeqNumber();
        return new Promise((resolve, reject): void => {
            //cLogger.debug(`push query send data state request to apsQueue. seqNr: ${seqNumber}`);
            const ts = 0;
    ...
    private async processApsQueue() {
        ...
        if (this.apsRequestFreeSlots !== 1) {
            cLogger.debug("no free slots. Delay sending of APS Request");
        ...
        switch (req.commandId) {
            case PARAM.PARAM.APS.DATA_REQUEST:
                ...
                if (readyToSend === false) { // wait until last request was confirmed or given time elapsed
                    cLogger.debug("delay sending of APS Request");
    ...
    private async processApsConfirmIndQueue() {
        ...
        switch (req.commandId) {
            case PARAM.PARAM.APS.DATA_INDICATION:
                //cLogger.debug(`read received data request. seqNr: ${req.seqNumber}`);
                if (this.DELAY === 0) {
            ...
            case PARAM.PARAM.APS.DATA_CONFIRM:
                //cLogger.debug(`query send data state request. seqNr: ${req.seqNumber}`);
                if (this.DELAY === 0) {
    ...
    private sendQueryDataStateRequest(seqNumber: number) {
        cLogger.debug(`DATA_CONFIRM - sending data state request - SeqNr. ${seqNumber}`);
    ...
    private sendReadReceivedDataRequest(seqNumber: number) {
        cLogger.debug(`DATA_INDICATION - sending read data request - SeqNr. ${seqNumber}`);
    ...
    private sendEnqueueSendDataRequest(request: ApsDataRequest, seqNumber: number) {
        ...
        cLogger.debug(`DATA_REQUEST - destAddr: 0x${dest} SeqNr. ${seqNumber} request id: ${request.requestId}`);
    ...
    private processApsBusyQueue() {
        ...
        while (i--) {
            ...
            if ((now - req.ts) > timeout) {
                cLogger.debug(`Timeout for aps request CMD: 0x${req.commandId.toString(16)} seq: ${req.seqNumber}`);
                //remove from busyQueue
