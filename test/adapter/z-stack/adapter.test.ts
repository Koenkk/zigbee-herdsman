...
describe("zstack-adapter", () => {
    ...
    it('Send zcl frame to all and include sleepy end devices', async () => {
        basicMocks();
        await adapter.start();

        mockZnpRequest.mockClear();
        const frame = Zcl.ZclFrame.create(Zcl.FrameType.GLOBAL, Zcl.Direction.CLIENT_TO_SERVER, true, null, 100, 'read', 0, [{attrId: 0}]);
        await adapter.sendZclFrameToAll(255, frame, 1, true);
        expect(mockQueueExecute.mock.calls[0][1]).toBe(undefined);
        expect(mockZnpRequest).toBeCalledTimes(1);
    ...
    it('Send zcl frame to all and dont include sleepy end devices', async () => {
        basicMocks();
        await adapter.start();

        mockZnpRequest.mockClear();
        const frame = Zcl.ZclFrame.create(Zcl.FrameType.GLOBAL, Zcl.Direction.CLIENT_TO_SERVER, true, null, 100, 'read', 0, [{attrId: 0}]);
        await adapter.sendZclFrameToAll(255, frame, 1, false);
        expect(mockQueueExecute.mock.calls[0][1]).toBe(undefined);
        expect(mockZnpRequest).toBeCalledTimes(1);
