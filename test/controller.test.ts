...
describe('Controller', () => {
    ...
    it('Join a device', async () => {
        ...
        expect(deepClone(await controller.getDevice('0x129'))).toStrictEqual(device);
    ...
    it('Get device should return same instance', async () => {
        ...
        await controller.start();
        await mockAdapterEvents['deviceJoined']({networkAddress: 129, ieeeAddr: '0x129'});
        expect(await controller.getDevice('0x129')).toBe(await controller.getDevice('0x129'));
    });
});