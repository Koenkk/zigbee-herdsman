...
import {Request} from "../src/controller/model/endpoint";
...
describe('Controller', () => {
        ...
        const expected = {
            ...
            "device":{
                ...
                "_endpoints":[
                    ...
                    "pendingRequests": [], "postponedRequests": [], "sendInProgress": false,
                    "_binds": [],
                    "_configuredReportings": [],
                    "meta":{},
    ...
    it('Write with sendWhen active when pending', async () => {
        ...
        const device = controller.getDeviceByIeeeAddr('0x129');
        const endpoint = device.getEndpoint(1);
        endpoint.pendingRequests.push(new Request(async () => {}, [], 100));
        mocksendZclFrameToEndpoint.mockClear();
        mocksendZclFrameToEndpoint.mockReturnValueOnce(null)
    ...
    it('Write with sendWhen active, discard messages after expiration', async () => {
        ...
        expect(mocksendZclFrameToEndpoint).toHaveBeenCalledTimes(0);
        expect(endpoint.pendingRequests.length).toBe(0);
        Date.now.mockReturnValue(150);
    });
    ...
    it('Implicit checkin while send already in progress', async () => {
        ...
        mocksendZclFrameToEndpoint.mockClear();
        const endpoint = device.getEndpoint(1);
        endpoint.pendingRequests.push(new Request(async () => {
    ...
    it('Fast polling', async () => {
        ...
        const endpoint = device.getEndpoint(1);
        endpoint.pendingRequests.push(new Request(async () => {}, [], 100, 'fastpoll', undefined, () => {}, () => {}));
        mocksendZclFrameToEndpoint.mockClear();
        mocksendZclFrameToEndpoint.mockReturnValueOnce(null);
